...
class Ensemble():
    ...
    def train(self, X_train, y_train, num_epochs, criterion=None, optimizers=None, lr=1e-5, wd=0, batch_size=None):
        if not criterion:
    ...
    def test(self, X_test, y_test, batch_size=None, models_used=None):
        ...
        with torch.no_grad():
            ...
            outputs = torch.zeros((models_used, X_test.shape[0], self.nb_classes))
    ...
    def train_test_on_the_fly(self, X_train, y_train, X_test, y_test, num_epochs, lr=1e-5, wd=0, batch_size=None, verbose=True):
        ...
        for epoch in range(num_epochs):
            train_acc, train_conf = self.train(X_train, y_train, 1, criterion=criterion, optimizers=optimizers, lr=lr, batch_size=batch_size)
            test_acc, test_conf = self.test(X_test, y_test, batch_size=batch_size)
